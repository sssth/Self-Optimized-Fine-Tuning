...
do  
    ...
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICES accelerate launch --main_process_port $main_process_port train.py \
        --train_type "SFT" --dataset_name $dataset --base_model $base_model_path
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICES accelerate launch --main_process_port $main_process_port inference.py  \
        --method "SFT" --inference_type "test" --dataset $dataset
    CUDA_VISIBLE_DEVICES=$last_cuda accelerate launch evaluate_batch.py \
        --method "SFT" --inference_type "test" --dataset $dataset
    
    # generate SD dataset
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICES accelerate launch --main_process_port $main_process_port inference.py \
        --method "SFT" --inference_type "generate" --dataset $dataset
    CUDA_VISIBLE_DEVICES=$last_cuda accelerate launch evaluate_batch.py  \
        --method "SFT" --inference_type "generate" --dataset $dataset
    python process.py --dataset $dataset --method "SFT" --process_type "mindist"
    ...
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICES accelerate launch --main_process_port $main_process_port inference.py \
        --method "SOFT-${alpha}" --inference_type "test" --dataset $dataset
    CUDA_VISIBLE_DEVICES=$last_cuda accelerate launch evaluate_batch.py \
        --method "SOFT-${alpha}" --inference_type "test" --dataset $dataset
    done
